...
def main():
    ...
    # ============================================================
    x_init = np.array([1.0, 0.0], dtype=float)

    sol = solve_ivp(
        fun=lambda t, x: drift_f(t, x, lam, m1, m2),
    ...
    for i in range(steps):
        Force_origin[i, :] = drift_f(0.0, path[i, :], lam, m1, m2)
    ...
    for i in range(len_LC):
        Force_LC[i, :] = drift_f(0.0, Limit_cycle[i, :], lam, m1, m2)
        Jacobian_LC[i, :, :] = jacobian_f(Limit_cycle[i, :], lam, m1, m2)
    ...
    for i in range(len_LC):
        ...
        # scipy solve_continuous_lyapunov(A, Q): A X + X A^T = -Q
