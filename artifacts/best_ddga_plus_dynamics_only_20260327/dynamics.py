...
class PredatorPreySensitive(Force):
    """Branch-sensitive predator-prey system with relaxation oscillations near critical kappa."""

    def __init__(self, alpha=0.1, beta=0.8, kappa=0.8794):
        self.alpha = alpha
        self.beta = beta
        self.kappa = kappa
    ...
    def force(self, state):
        ...
        prey = torch.clamp(state[..., 0], min=0.0)
        predator = torch.clamp(state[..., 1], min=0.0)
        denom = prey + predator

        zeros = torch.zeros_like(denom)
        interaction = torch.where(denom <= 1e-12, zeros, prey * predator / denom)
        predator_gain = torch.where(denom <= 1e-12, zeros, self.kappa * prey * predator / denom)

        dprey = self.alpha * prey * (1.0 - prey) - interaction
        dpredator = -self.beta * predator + predator_gain
        drift = torch.stack([dprey, dpredator], dim=-1)
        return drift.squeeze(0) if squeeze_output else drift


class PredatorPreyExponential(Force):
    """Predator-prey system with exponential functional response and a robust limit cycle."""

    def __init__(self, r=2.0, k=3.0, beta=0.5, mu=1.2, death_rate=0.4):
        self.r = r
        self.k = k
        self.beta = beta
        self.mu = mu
    ...
    def force(self, state):
        ...
        prey = torch.clamp(state[..., 0], min=0.0)
        predator = torch.clamp(state[..., 1], min=0.0)
        exp_term = torch.exp(-self.beta * prey)

        dprey = self.r * prey * (1.0 - prey / self.k) - prey * exp_term * predator
        dpredator = predator * (-self.death_rate + self.mu * prey * exp_term)
        drift = torch.stack([dprey, dpredator], dim=-1)
        return drift.squeeze(0) if squeeze_output else drift
...
def get_force_(force_type, **kwargs):
    ...
    force_ = {
        ...
        'PredatorPreySensitive': PredatorPreySensitive,
        'PredatorPreyExponential': PredatorPreyExponential,
    }
